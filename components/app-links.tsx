    ...
    <div className={`space-y-3 ${className}`}>
      {title && (
        <h3 className={`text-lg font-semibold mb-4 whitespace-nowrap ${
          isForceMode ? "text-white" : "text-gray-900 dark:text-white"
        }`}>{title}</h3>
      )}
        ...
        {links.map((link) => (
          ...
          >
            ...
            <div className={`w-px h-8 mx-1 ${
              isForceMode ? "bg-gray-500" : "bg-gray-300 dark:bg-gray-600"
            }`}></div>
            <div className="flex flex-col text-left">
              <span className={`text-xs leading-tight ${
                isForceMode ? "text-gray-300" : "text-gray-600 dark:text-gray-300"
              }`}>
