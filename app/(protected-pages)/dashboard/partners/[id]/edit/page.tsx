...
export default function EditPartnerPage() {
    ...
    const fetchPartner = async () => {
      try {
        ...
        if (!response.ok || !data.success) {
          throw new Error(data.error || 'Parceiro não encontrado');
        }
        ...
        setProductsCount(partner.productsCount || 0);
      } catch (err) {
        console.error('Error fetching partner:', err);
        const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar parceiro';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao atualizar parceiro';
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setLoading(false);
